...
if (NOT DEFINED OF_ROOT)
    execute_process(COMMAND ${PROJECT_SOURCE_DIR}/../../.. OUTPUT_VARIABLE OF_ROOT OUTPUT_STRIP_TRAILING_WHITESPACE)
endif ()
...
if (CMAKE_C_COMPILER MATCHES ".+/emcc$")
    set(PLATFORM_OS "emscripten")
    #PLATFORM_OS=emscripten
endif ()
if (NOT DEFINED PLATFORM_VARIANT)
    execute_process(COMMAND uname -s OUTPUT_VARIABLE PLATFORM_OS OUTPUT_STRIP_TRAILING_WHITESPACE)
endif ()
...
file(GLOB GLOB_TEST ${RPI_ROOT}/etc/*-release)
if (GLOB_TEST MATCHES "/etc/os-release")
    execute_process(COMMAND grep ID=raspbian ${RPI_ROOT}/etc/*-release OUTPUT_VARIABLE CMD_RES OUTPUT_STRIP_TRAILING_WHITESPACE)
    if (CMD_RES MATCHES "ID=raspbian")
        set(IS_RASPBIAN true)
    endif ()
endif ()

if (DEFINED IS_RASPBIAN)
    set(PLATFORM_ARCH "armv6l")
    set(HOST_ARCH "armv6l")
    if (DEFINED RPI_ROOT)
    ...
    if (NOT DEFINED PLATFORM_ARCH)
        ...
        execute_process(COMMAND uname -m OUTPUT_VARIABLE PLATFORM_ARCH OUTPUT_STRIP_TRAILING_WHITESPACE)
    endif ()
    if (NOT DEFINED CROSS_COMPILING)
        if (NOT (HOST_ARCH MATCHES ${PLATFORM_ARCH}))
            set(CROSS_COMPILING true)
        else ()
...
if (CMAKE_BUILD_TYPE MATCHES "Debug")
    ...
    message(HOST_OS=${HOST_OS})
    message(CROSS_COMPILING=${CROSS_COMPILING})
    message(PLATFORM_VARIANT=${PLATFORM_VARIANT})
    message(IS_RASPBIAN=${IS_RASPBIAN})
endif()

# if not defined, construct the default PLATFORM_LIB_SUBPATH
if (NOT DEFINED PLATFORM_LIB_SUBPATH)
    # determine from the arch
    if (PLATFORM_OS MATCHES "Linux")
        if (PLATFORM_ARCH MATCHES "x86_64")
            set(PLATFORM_LIB_SUBPATH "linux64")
        elseif (PLATFORM_ARCH MATCHES "armv6l")
            set(PLATFORM_LIB_SUBPATH "linuxarmv6l")
        elseif (PLATFORM_ARCH MATCHES "armv7l")
            set(PLATFORM_LIB_SUBPATH "linuxarmv7l")
        elseif (PLATFORM_ARCH MATCHES "i386")
            set(PLATFORM_LIB_SUBPATH "linux")
        elseif (PLATFORM_ARCH MATCHES "i686")
            set(PLATFORM_LIB_SUBPATH "linux")
        else ()
            message(SEND_ERROR "This CMake file does not support your architecture ${PLATFORM_ARCH}")
        endif ()
